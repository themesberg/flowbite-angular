...
import type { SidebarToggleBaseTheme, SidebarToggleClass, SidebarToggleProperties } from './sidebar-toggle.theme';

import { inject, InjectionToken } from '@angular/core';
import { twMerge } from 'tailwind-merge';

export const FLOWBITE_SIDEBAR_TOGGLE_THEME_TOKEN = new InjectionToken<SidebarToggleBaseTheme>(
...
export class SidebarToggleThemeService implements FlowbiteThemeService<SidebarToggleProperties> {
  ...
  public getClasses(properties: SidebarToggleProperties): SidebarToggleClass {
    ...
    const output: SidebarToggleClass = {
      rootClass: twMerge(theme.base, theme.color[properties.color], theme.size[properties.color]),
    };
