...
import type { BreadcrumbBaseTheme, BreadcrumbClass, BreadcrumbProperties } from './breadcrumb.theme';

import { inject, InjectionToken } from '@angular/core';
import { twMerge } from 'tailwind-merge';

export const FLOWBITE_BREADCRUMB_THEME_TOKEN = new InjectionToken<BreadcrumbBaseTheme>(
...
export class BreadcrumbThemeService implements FlowbiteThemeService<BreadcrumbProperties> {
  ...
  public getClasses(properties: BreadcrumbProperties): BreadcrumbClass {
    const theme: BreadcrumbBaseTheme = mergeTheme(this.baseTheme, properties.customStyle);
